...
TIM_HandleTypeDef a_timx_npwm_chy_handle;     /* 定时器x句柄 */
...
{		
    ...
    a_timx_npwm_chy_handle.Instance = ATIM_TIMX_NPWM;                  /* 定时器x */
    ...
    a_timx_npwm_chy_handle.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_ENABLE; /*使能TIMx_ARR进行缓冲 */
    ...
    HAL_TIM_PWM_ConfigChannel(&a_timx_npwm_chy_handle, &timx_oc_npwm_chy, ATIM_TIMX_NPWM_CHY); /* 配置TIMx通道y */
    HAL_TIM_PWM_Start(&a_timx_npwm_chy_handle, ATIM_TIMX_NPWM_CHY);    /* 开启对应PWM通道 */

    HAL_NVIC_SetPriority(ATIM_TIMX_NPWM_IRQn, 1, 3);                   /* 设置中断优先级，抢占优先级1，子优先级3 */
    HAL_NVIC_EnableIRQ(ATIM_TIMX_NPWM_IRQn);                           /* 开启ITMx中断 */
    HAL_TIM_Base_Start_IT(&a_timx_npwm_chy_handle);       						 /* 允许更新中断 */
}
...
void ATIM_TIMX_NPWM_IRQHandler(void)
{
    ...
    if(__HAL_TIM_GET_FLAG(&a_timx_npwm_chy_handle, TIM_FLAG_UPDATE) != RESET)
    {
