...
void bsp_InitDAC8562(void)
{
	spi1_init();
	//复位
	DAC8562_WriteCmd((5 << 19) | (0 << 16) | (1 << 0));
	printf("reset:\r\n");
...
int32_t DAC8562_DacToVoltage(uint8_t _ch, uint16_t _dac)
{
	...
	if (_ch == 0){
			return CaculTwoPoint(XA1, YA1, XA2, YA2, _dac);
	}else if (_ch == 1){
			return CaculTwoPoint(XB1, YB1, XB2, YB2, _dac);
	}else{
			return 0;
	}
}
...
uint32_t DAC8562_VoltageToDac(uint8_t _ch, int32_t _volt)
{
	/* CaculTwoPoint(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x);*/
	if (_ch == 0){
			return CaculTwoPoint(YA1, XA1, YA2, XA2, _volt);
	}else if (_ch == 1){
			return CaculTwoPoint(YB1, XB1, YB2, XB2, _volt);
	}else{
			return 0;
	}
}
