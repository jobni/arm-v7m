...
void spi1_init(void)
{
    ...
    g_spi1_handler.Init.CLKPhase = SPI_PHASE_2EDGE;                    /* 串行同步时钟的第一个跳变沿（上升或下降）数据被采样 */
    g_spi1_handler.Init.NSS = SPI_NSS_SOFT;                            /* NSS信号由硬件（NSS管脚）还是软件（使用SSI位）管理:内部NSS信号有SSI位控制 */
    g_spi1_handler.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_8;   /* 定义波特率预分频的值:波特率预分频值为8 */
 ...
 * @retval      无
 */
uint8_t spi1_write_data(uint8_t *tx_data,uint16_t size)
{
    uint16_t retry = 0;
    uint16_t i;
    ...
    for (i = 0;i < size;i++)
    {
        ...
        SPI1_SPI->DR = tx_data[i];          /* 发送一个byte */
        retry = 0;
    }
