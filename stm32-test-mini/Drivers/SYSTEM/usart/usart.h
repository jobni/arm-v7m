...
extern UART_HandleTypeDef g_uart2_handle;       /* HAL UART2句柄 */

extern uint8_t  g_usart1_rx_buf[USART_REC_LEN];  /* 接收缓冲,最大USART_REC_LEN个字节.末字节为换行符 */
extern uint16_t g_usart1_rx_sta;                 /* 接收状态标记 */
extern uint8_t  g_usart2_rx_buf[USART_REC_LEN];  /* 接收缓冲,最大USART_REC_LEN个字节.末字节为换行符 */
extern uint16_t g_usart2_rx_sta;                 /* 接收状态标记 */
extern uint8_t g_rx_buffer1[RXBUFFERSIZE];       /* HAL库USART1接收Buffer */
extern uint8_t g_rx_buffer2[RXBUFFERSIZE];       /* HAL库USART2接收Buffer */


void usart1_init(uint32_t bound);                /* 串口1初始化函数 */
void usart2_init(uint32_t bound);                /* 串口2初始化函数 */
void usart_check();
#endif
